...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: bgColor }}>
      {/* 좌측 사이드바 */}
      <aside
        className="w-16 sm:w-56 flex flex-col shrink-0 border-r"
        style={{
          backgroundColor: "#fff",
      ...
      >
        {/* 로고 */}
        <div className="px-4 py-5 border-b" style={{ borderColor: "rgba(0,0,0,0.06)" }}>
          <div className="hidden sm:flex items-center gap-2.5">
            ...
            >
              <svg width="12" height="14" viewBox="0 0 12 14" fill="none">
                <path d="M6 1C6 1 2 5.5 2 8.5C2 10.99 3.79 13 6 13C8.21 13 10 10.99 10 8.5C10 5.5 6 1 6 1Z" fill="white"/>
              </svg>
            </div>
        ...
        <div className="p-3 border-t" style={{ borderColor: "rgba(0,0,0,0.06)" }}>
          <div className="hidden sm:flex items-center gap-2.5 px-2 py-1.5 mb-1 rounded-xl hover:bg-gray-50 transition-colors cursor-default">
            <div
          ...
          <button
            onClick={handleSignOut}
            className="btn-jelly w-full flex justify-center sm:hidden text-gray-400 hover:text-gray-600 py-1 rounded-lg hover:bg-gray-50"
          >
            {Icons.logout}
          </button>
        </div>
      ...
      <main className="flex-1 overflow-hidden">
        ...
        {activeTab === "settings" && (
          <SettingsTab userId={userId} profile={profile} onThemeUpdate={handleThemeUpdate} />
        )}
      </main>
    </div>
  );
